...
CC0-1.0 and Unicode-3.0. The last entry is not a concession: the Unicode license carries no
copyleft, and the internationalisation family that arrives through url and idna is
unavoidable in any project that parses a link or normalises text. MPL-2.0 is \req{not} on it. Its copyleft is file scoped rather than project
...
\begin{description}[style=nextline,leftmargin=0pt]
  \item[Pin the UI framework to its published release.] gpui is taken from the registry at an
  exact published version under Apache~2.0, not from the upstream editor's source tree. This
  distinction is load bearing and was discovered while implementing it: in that tree the path
  \ctl{gpui} to \ctl{sum\_tree} to \ctl{ztracing} reaches GPL-3.0-or-later code, which would
  put a disclosure obligation on this binary and defeat the reason the standalone product
  exists. The published crate does not carry that edge. Upstream versions are adopted
  deliberately, never automatically, which removes the rebase treadmill that characterises the
  fork.

  \item[Judge a dependency by whether it ships.] A build dependency executes during
  compilation and never links into the artefact, so its obligations do not follow the binary.
  The gate records such cases as named exceptions carrying their reason rather than widening
  the allow list for everything.
