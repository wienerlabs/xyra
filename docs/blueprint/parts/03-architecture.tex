...
\toprule
\textbf{Crate} & \textbf{Kind} & \textbf{Responsibility} \\
\midrule
\endfirsthead
\toprule
\textbf{Crate} & \textbf{Kind} & \textbf{Responsibility} \\
\midrule
\endhead
xyra-ui & library & Element model wrapper over the vendored UI framework, theme tokens, primitives \\
xyra-text & library & Rope buffer, transactions, undo history, multi cursor selection algebra \\
xyra-syntax & library & Parser host, incremental reparse, highlight queries, structural selection \\
xyra-lang & library & Language server client, capability negotiation, diagnostics, completion \\
xyra-project & library & Worktree model, ignore semantics, file watching, path resolution \\
xyra-vcs & library & Git operations, status, diff, blame, staging, commit, branch \\
xyra-term & library & Terminal grid, PTY supervision, escape parsing, search \\
xyra-context & library & Symbol and import graph, retrieval, compression pipeline, budget solver \\
xyra-memory & library & Durable stores, caches, journal, mission state, migrations \\
xyra-agent & library & Agent process supervision, protocol clients, tool exposure, transcript capture \\
xyra-mission & library & Ticket graph, supervisor loop, failure policy, budgets \\
xyra-verify & library & Sandbox execution, visual verification, quality assurance sweeps, flags \\
xyra-fleet & library & Multi repository registry, cross repository search and impact \\
xyra-skills & library & Skill registry, resolution, injection, precedence \\
xyra-ext & library & Extension host, capability enforcement, manifest validation \\
xyra-proto & library & Message types shared between the interface and the core \\
xyrad & binary & Core service binary, wiring, supervision tree \\
xyra & binary & Interface binary, surfaces, keymap, command registry \\
\bottomrule
